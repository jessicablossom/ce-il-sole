...
export const isNightInCentralEurope = (date: Date): boolean => {
  const hour = getHourInTimeZone(date, CENTRAL_EUROPEAN_TIME_ZONE);

  return hour >= NIGHT_START_HOUR || hour < NIGHT_END_HOUR;
};

export const getDayPeriodInCentralEurope = (date: Date): DayPeriod =>
  isNightInCentralEurope(date) ? "night" : "day";
